...
temp_data <- read.csv("./data/erdap_griddap_lakeChamplain_temp_2024.csv")
nrow(temp_data) #2419999
```
...
nrow(temp_clean) # 557473; reduced from orginal set
...
july_data <- joined_sf %>% 
  filter(month == "07-2024")

# Create the map
ggplot(july_data) +
  geom_sf(aes(fill = avg_temp), color = "black") + 
  scale_fill_gradient(low = "#67a9cf", high = "#ef8a62", name = "Average temperature (°C)") +
  labs(title = "Average temperature in July") +
  theme_bw() 
```
Output joined_sf so I can use it in other scripts:
```{r}
# st_write(joined_sf, "2024_avg_temp_data.shp", driver = "ESRI Shapefile")
# write.csv(st_drop_geometry(joined_df), "2024_avg_temp_data.csv", row.names = FALSE)
```