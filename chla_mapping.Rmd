...
```
How does chlorphyll concentration change in different parts of the lake?
```{r}
...
ggplot(april_data) +
  ...
  scale_fill_viridis_c(name = "Average Chlorophyll-a (mg m^-3)") +
  labs(title = "Average Chlorophyll-a in April") +
  theme_bw() 
```
How does chlorophyll-a change in each region of the lake month by month?
```{r}
ggplot(joined_df, aes(x = month, y = avg_chla, color = regions)) +
  geom_point() +
  geom_line(aes(group = regions)) +
  labs(title = "Monthly average chlorophyll by lake region",
       x = "Month", y = "Average chlorophyll (mg m^-3)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

Output joined_sf so I can use it in other scripts:
```{r}
# st_write(joined_sf, "2024_avg_chla_data.shp", driver = "ESRI Shapefile")
# write.csv(st_drop_geometry(joined_df), "2024_avg_chla_data.csv", row.names = FALSE)
```