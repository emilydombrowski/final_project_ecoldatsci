...
weights <- seq(0.5, 1.50, by = 0.01)  # Weight range (grams)

# Create a data frame with blank columns for additional scenarios
data <- data.frame(
  weight = weights,
  consumption_rate = NA,
  respiration_rate = NA,
  highly_competitive_quagga = NA,
  weakly_competitive_quagga = NA
)
...
ggplot(results, aes(x = weight, y = growth, color = basin)) +
  geom_line() +
  labs(title = "Zebra mussel growth potential by lake basin and weight",
